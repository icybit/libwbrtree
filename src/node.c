...
{
	/*TODO: DETERMINE OPTIMAL SPLIT DIMENSION (AXIS) -- USE QSORT AS THE SORTING ALGORITHM*/

	/*void **x_axis = malloc(NELEMS(entries) * sizeof(void *));
	void **y_axis = malloc(NELEMS(entries) * sizeof(void *));

	memcpy(x_axis, entries, NELEMS(entries));
	memcpy(y_axis, entries, NELEMS(entries));

	qsort(x_axis, NELEMS(x_axis), sizeof(void *), entry_compare_x);
	qsort(y_axis, NELEMS(y_axis), sizeof(void *), entry_compare_y);*/
...
{
	/*TODO: PERFORM OPTIMAL ENTRY DISTRIBUTION BETWEEN GROUPS*/
}
